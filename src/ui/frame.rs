...
pub fn logical_width(window_width: f32, window_height: f32) -> f32 {
    if window_height <= 0.0 || !window_width.is_finite() || !window_height.is_finite() {
        return DESIGN_WIDTH;
    }
    let aspect = (window_width / window_height).clamp(MIN_ASPECT, MAX_ASPECT);
    (HEIGHT * aspect).round()
}
...
mod tests {
    ...
    fn a_sixteen_by_nine_window_gets_the_design_width() {
        assert!((logical_width(1920.0, 1080.0) - DESIGN_WIDTH).abs() < 1.0);
        assert!((logical_width(1280.0, 720.0) - DESIGN_WIDTH).abs() < 1.0);
    }
    ...
    fn a_wider_window_gets_a_wider_space_rather_than_a_bigger_picture() {
        ...
        let wide = logical_width(1920.0, 900.0);
    ...
    fn a_narrower_window_gets_a_narrower_space() {
        let narrow = logical_width(1024.0, 768.0);
    ...
    fn extreme_shapes_are_clamped_rather_than_obeyed() {
        ...
        let absurdly_wide = logical_width(10_000.0, 500.0);
        assert!((absurdly_wide / HEIGHT - MAX_ASPECT).abs() < 0.01);

        let portrait = logical_width(400.0, 900.0);
        assert!((portrait / HEIGHT - MIN_ASPECT).abs() < 0.01);
    }
    ...
    fn extra_width_goes_to_the_reels() {
        ...
        let narrow = Frame::new(1100.0);
        let wide = Frame::new(1680.0);
    ...
    fn a_top_pinned_overlay_keeps_its_top_and_stays_on_screen() {
        let frame = Frame::new(DESIGN_WIDTH);
