...
impl GameData {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        validate_free_spins(self)?;
        validate_gamble_and_cascade(self)?;
        validate_seam(self)
    }
}

fn validate_board(data: &GameData) -> Result<(), String> {
    let reel_count = data.config.reel_count;
    let row_count = data.config.row_count;
    ...
    // can be six wide.
    if data.config.evaluation != Evaluation::Cluster && reel_count > MAX_RUN {
        return Err(format!(
            "reel_count {} exceeds the paytable maximum run of {}",
            reel_count, MAX_RUN
        ));
    }
    if data.reels.len() != reel_count {
        return Err(format!(
    ...
    if data.symbols.wild().is_none() {
        return Err("no wild symbol declared in symbols.json".to_owned());
    }
    if data.symbols.scatter().is_none() {
        return Err("no scatter symbol declared in symbols.json".to_owned());
    }
    Ok(())
}

fn validate_economy(data: &GameData) -> Result<(), String> {
    if data.config.line_bets.is_empty() {
        return Err("game_config.json declared no line bets".to_owned());
    }
    if data.config.line_bets.iter().any(|bet| *bet <= 0) {
...
fn validate_features(data: &GameData) -> Result<(), String> {
    ...
    validate_feature_buy(&data.featurebuy, cells)?;
    if let Some(refine) = &data.freespins.refine {
        ...
        for id in &refine.order {
            let Some(index) = data.symbols.index_of(id) else {
            ...
            if data.symbols.is_wild(index)
                || data.symbols.is_scatter(index)
                || Some(index) == data.symbols.hoard()
            {
    ...
    if let Some(heights) = data.config.reel_heights {
        ...
        // that are not there.
        if data.config.evaluation == Evaluation::Lines {
...
fn validate_free_spins(data: &GameData) -> Result<(), String> {
    ...
    if let Some(first) = data.freespins.shapes.first() {
...
fn validate_gamble_and_cascade(data: &GameData) -> Result<(), String> {
    if data.gamble.max_steps == 0 {
        return Err("a gamble with no steps could never be taken".to_owned());
    }
    if data.gamble.ceiling_multiple <= 0 {
...
fn validate_seam(data: &GameData) -> Result<(), String> {
    let cells = data.config.reel_count * data.config.row_count;
    if data.holdspin.trigger_eggs == 0 || data.holdspin.trigger_eggs > cells {
        return Err(format!(
            "holdspin trigger_eggs ({}) must fit on a {}-cell grid",
            data.holdspin.trigger_eggs, cells
        ));
    }
    ...
    if data.seam.trigger_count == 0 || data.seam.trigger_count > cells {
        return Err(format!(
            "seam trigger_count ({}) must fit on a {}-cell grid",
            data.seam.trigger_count, cells
        ));
    }
    if data.seam.rites.is_empty() {
        return Err("seam.json declared no rites".to_owned());
    }
    if data.seam.steps == 0 {
        return Err("a seam with no steps would never work the board".to_owned());
    }
    if data.seam.max_multiple <= 0 {
