...
mod batch {
    ...
    fn every_machine_holds_its_rtp_over_a_long_run() {
        // Every cabinet is measured before any of them is judged. Asserting
        // inside the loop meant the first machine out of band hid the other
        // five, and a retune is a decision about the *catalog* — knowing which
        // way the other cabinets are drifting is most of the information.
        ...
        for machine in crate::data::MACHINES {
            ...
            println!("{:>9}: {}", machine.id, report.summary());
            if (report.rtp() - TARGET_RTP).abs() >= FULL_TOLERANCE {
                adrift.push(format!("{} at {:.4}", machine.id, report.rtp()));
            }
        }
