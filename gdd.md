...
### 5.65 What the shapes actually feel like (post-v1)

§5.64 offers a choice and proves it is fair with arithmetic. Arithmetic is not
reassurance. A player looking at *"15 spins at ×2"* beside *"6 spins at ×5"* has
been handed two numbers and asked to trust a third they cannot see — and the
number they actually care about is not the mean at all.

So both shapes are now **measured**, two thousand features each, the same way the
cabinets (§5.17) and the buy tiers (§5.22) already are:

| cabinet | shape | mean | pays nothing | best seen |
|---|---|---|---|---|
| Dragon's Hoard | Long Hunt | 50.9x | **0.0%** | 840x |
| Dragon's Hoard | Swift Kill | 53.5x | **7.1%** | **1,927x** |
| Emberfall | Long Hunt | 42.7x | 0.0% | 537x |
| Emberfall | Swift Kill | 43.0x | 0.3% | 1,055x |
| Avalanche | Long Hunt | 28.0x | 0.2% | 160x |
| Avalanche | Swift Kill | 27.8x | 2.0% | 266x |
| Tidepool | Long Hunt | 59.7x | 0.0% | 193x |
| Tidepool | Swift Kill | 58.5x | 1.7% | 333x |

**The means land together and everything else does not**, which is exactly the
trade the design claimed and could not previously show. On Dragon's Hoard the
Long Hunt has never once come back with nothing in two thousand runs; the Swift
Kill does it seven times in a hundred, and pays more than double at the top. That
is a real decision about what kind of evening you want, and it is now legible at
the moment it is made: each button carries `blank 7% · best 1927x` under it.

**Granted rather than triggered.** The profiler starts each run outright instead
of waiting for scatters, because what is being compared is the *feature* and
waiting would spend a thousand paid spins per measured run. Both shapes get the
same award for the same reason — it is a like-for-like comparison or it is
nothing.

**And it is the other half of §5.64's proof.** That the shapes are worth the same
is enforced by construction and by a test on the award table; this runs two
thousand of them each way and watches the means arrive together. If they ever do
not, something about the feature is not linear in the multiplier and the whole
premise is wrong — which is a thing worth being able to find out.

### 5.66 The gate that did not fire (post-v1)

§5.29 opens with a promise:

> Add a cabinet with a mechanic and forget to describe it and the build fails
> naming the topic — which is the failure that should have fired three iterations
> ago and could not, because there was nothing to compare the prose against.

§5.64 added a mechanic. The free-spin run can now be traded short and sharp, and
**nothing in the game said so**. No rule, no paytable line, nothing. The build
did not fail. Two iterations shipped a decision the cabinet never mentioned,
which is the exact fault §5.29 was written to make impossible.

**The hole is that `Topic::present` is a hand-written list.** It compares the
prose against a set of topics and cannot notice a mechanic nobody added a topic
for. That is the same shape as §5.50's screen registry and §5.53's copy of it in
the harness: *a list that must be maintained is a list that goes stale, and
staleness looks exactly like correctness.*

So the question is turned round. Every key in the free-spin data must be declared
either **a mechanic** — named by a topic that has to be present whenever the key
is switched on — or **tuning**, excused in writing. A key that is neither fails
the test by name. Adding a field to `freespins.json` now forces a decision
instead of permitting silence: the default is "explain this", and getting out of
it means saying why somewhere a reader will see.

Disproved by deleting the `shapes` entry, which reproduces exactly what shipped:

```
dragon: freespins.json carries 'shapes' and nothing here says whether it is a
mechanic that must be explained or tuning that need not be.
```

**Then writing the missing rule broke the panel, twice.** The prose gate caught
it as an essay at 420 characters, which is a good rule and was correctly applied
to me. And with a thirteenth topic, Wyrmspire — six mechanics, the most of any
cabinet — no longer fitted two columns even at the minimum readable size. §5.60
had already widened the panel once for §5.59's line diagrams.
...
### 5.68 That is the session (post-v1)

§5.30 lets a player cap their losses, their time or their spins, and the caps
work — play stops the moment one binds. What happened *then* was that the SPIN
button went grey and a notification said:

> You have reached your 5,000 credit loss limit. Start a new game to play on.

That was all of it. A toast, gone in a few seconds, telling someone who had just
hit a limit **they set for themselves** how to get around it. The single moment
where this game most owes a player a clear account of what happened, and it was a
line of disappearing text with a workaround in it.

So there is a screen now, and it is an account rather than a funnel:

```
That is the session
You set a 20-minute limit on this session, and it is up.
...
Came back                                               10,449
Net                                                     -7,751
Best single win                                          1,240

One session is far too little play to say anything about the cabinet.
```

**Every part of that is a decision.** The cap is stated first and in the player's
own terms — *you set this* — because they did, and they are owed the reason
before they are shown anything else. The figures come from the same
`SessionClock` the cap was measured against, so the summary cannot be a second
opinion about the thing that just stopped play. The caveat about sample size is
the one the Ledger makes at length (§5.18), said once here rather than dropped.

And "New session" is second, smaller, and on the other side of the panel. It
exists because the caps are the player's own and §5.30's asymmetry already
governs what happens to them — a loosened cap waits for the next session, a
tightened one binds at once — but a screen that led with it would be selling the
next session instead of closing this one.

Closing it puts the account away without pretending the cap has lifted: the reels
stay stopped, because the cap is what stopped them.

**Registering it cost one line and bought the whole audit.** `Screen::SessionOver`
joined the registry (§5.50) and the eighteenth screen was swept at three text
sizes, six themes, three widths and a 40% translation without the harness being
...
### 5.73 The hints that told you to press a key (post-v1)

§5.72 gave every screen a door and then did not look one line down at the hint
bar, where the game does its actual teaching. Every hint there began **"Press
R"**, **"Press G"**, **"Press C"**, **"Press L"** or **"Press B"** — advice that
on a touch device is not merely unhelpful but a dead end, since the panel it
names had no other way in. The fix and the fault were shipped in the same
iteration.

And the machines hint read *"There are **five**, and they are five different
games rather than five themes."* Tidepool made it six in §5.35. A sentence
holding a number is a sentence with an expiry date on it.

Both are the same failure: **a hint that repeats something instead of deriving
it**. So a hint now carries a `screen` id, and the bar renders an **Open**
button that goes there — looked up in the registry, validated against the list
the menu offers, so a hint can only point somewhere a player can actually reach.
Counts come from `{cabinets}`, substituted from `MACHINES.len()` and *spelled*,
because "there are six" is prose and "there are 6" is a readout. The validator
refuses a hint that names an unreachable screen and refuses one that spells out
a number itself.

Two of the five sentences failed their own new tests immediately: the gamble
hint pointed at a screen the game deals rather than offers, and the buy hint
named "Buy" while the panel it opened was called "Buy a feature".

**The capture scene is the more interesting half.** It photographed whichever
hint happened to be first due, which for six sections was the shortest one. It
now holds the **longest** — the only one that can wrap into the bar's own
border. Getting that picture took three goes, and each failure was the same
shape as the last:

1. Dismiss the others, and the longest one **was not due** — so the bar fell
   back to the shortcut line. A capture that looks perfectly composed and shows
   nothing at all.
2. Force the counter, and the sixty scripted spins that run afterwards **wrote
   it back**. Ordering, not logic.
3. Force it last, and the hint was still absent, because the save on this
   machine had already played two cabinets and thereby **earned** the hint about
   cabinets. Due is not enough; it has to be due *and* unearned.

Then the picture arrived and the sentence wrapped, its second line clipped by
the border. The cap had been measured at 124 characters on a bar **with no Open
button on it** — the button costs ninety pixels, the text box is 576px rather
than 668px with one there, and the same sentence no longer fits. A budget
measured on the easy case is not a budget. It is 105 now, taken with the door.

Nothing here was found by reading the code. All of it came from looking at the
strip of pixels the player actually reads, which is the same lesson as §5.59 and
§5.61 and is apparently one that has to be learned per surface.

The scene file crossed 800 lines on the way and split: `capture_scenes.rs` keeps
the `match` on scene names that reads like a list of pictures, and
...
### 5.79 The layout turns (post-v1)

§5.78 ended by saying a tablet held upright cannot be fixed by resizing
controls. That was true and it was the wrong conclusion. **The layout was what
needed to turn.**

§5.46 wrote it down without noticing: *"The height stays 720."* Below 4:3 the
aspect was clamped and the letterbox came back. The frame did not refuse
portrait so much as have no way to express one — a fixed height is a landscape
assumption in the shape of a constant, and forty-nine files had inherited it.

**Portrait is 720 wide and as tall as the window.** The same 720 the landscape
layout uses for its own short side, so a control is the same size in logical
pixels either way. And that is the whole payoff: 810 CSS pixels against a
720-wide layout puts a 44-pixel control at 49. The touch audit agrees —
**"need a 720px-wide window"**, which is the floor. A tablet held upright clears
the standard for the first time, and it does so because the layout got
*narrower*, not because anything got bigger.

The turn itself: reels across the top, controls underneath, header in two rows
because its four buttons and three badges want 946 pixels and a turned frame is
720 across — the landscape anchors put them off the left edge entirely. The
session buttons go four abreast instead of two-by-two, which hands 52 pixels
back to the reel window. The reel grid keeps its cells square instead of filling
its box, because five reels of three rows drawn to fill a wide short box came
out as fifteen letterbox slits: the symbols were all still there and correct,
and nobody could tell a gem from a coin.

**Landscape is byte-for-byte what it was**, and there is a test that says so
against every one of the four regions. That was the real risk in this change and
it is the one thing worth asserting directly.

**What is not done, plainly.** Portrait still reports eleven layout findings,
all of them in overlay panels written against a 1280-wide screen — the rules
panel is 1180 wide and lays out in three columns, so on a 720 frame its prose
runs past the edge. The reel window is smaller than it should be, because the
controls take 430 of the 700 available and a two-column portrait wager panel
would take half that. The gates cover landscape only; portrait is measured and
reported here rather than enforced.

So the honest state is: the layout turns, every control is reachable and clears
the touch standard, and the panels that open on top of it have not been turned
yet.
...
| A layout that only exists one way up | The frame turns below 4:3 — reels above, controls below, header in two rows (§5.79). A tablet held upright clears the touch standard because the layout got narrower. Overlay panels are not turned yet. |
